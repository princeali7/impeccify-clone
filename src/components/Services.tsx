...
export default function Services() {
  ...
  useEffect(() => {
    ...
    rowRefs.current.forEach((row) => {
      if (!row) return;
      const text = row.querySelector<HTMLElement>('[data-animate="text"]');
      const image = row.querySelector<HTMLElement>('[data-animate="image"]');
      const isReversed = row.dataset.reversed === 'true';

      const obs = new IntersectionObserver(
        ([entry]) => {
          if (entry.isIntersecting) {
            if (text) {
              text.classList.remove('slide-hidden');
              text.classList.add(isReversed ? 'animate-slide-in-right' : 'animate-slide-in-left');
            }
            if (image) {
              image.classList.remove('slide-hidden');
              image.classList.add(isReversed ? 'animate-slide-in-left' : 'animate-slide-in-right');
            }
            obs.disconnect();
          }
        },
    ...
    <section id="services" className="relative py-9 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <span className="px-6 py-2 bg-[#151515] mx-auto block max-w-[160px] rounded-full">Our Services</span>
          <h2 className="text-3xl md:text-5xl font-bold mt-3 md:mt-5 mb-4">Web Design and Development Services</h2>
          <p className="text-gray-400 text-lg max-w-[370px] md:max-w-[550px] mx-auto">
        ...
        <div className="space-y-24">
          {serviceData.map((service, idx) => (
            <div
              ...
              data-reversed={idx % 2 === 1 ? 'true' : 'false'}
              className={`flex flex-col ${idx % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-12 items-center`}
            >
              {/* Text Content */}
              <div data-animate="text" className="flex-1 slide-hidden">
              ...
              {/* Real Service Image */}
              <div data-animate="image" className="flex-1 w-full slide-hidden">
