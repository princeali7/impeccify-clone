...
export default function ToolsPage() {
  ...
  // Highlight the category card whose section is in view
  useEffect(() => {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-black pt-24 pb-20">
                ...
                <button
                  ...
                  className={`group text-left flex flex-col gap-4 p-5 rounded-2xl border transition-all duration-200 active:scale-[0.97] ${
                    isActive
                      ...
                      : 'bg-white/2.5 border-white/[0.07] hover:bg-white/5 hover:border-white/15'
                  }`}
                >
                  {/* Icon */}
        ...
        {/* Tool sections — all always visible */}
            ...
            <section key={cat.id} id={cat.id} className="scroll-mt-24">
              ...
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${meta?.iconBg ?? 'bg-white/10 text-gray-400'}`}>
                    <span className="[&>svg]:w-4 [&>svg]:h-4">{meta?.icon}</span>
                  </div>
                  <div>
                    <h2 className="text-base font-bold text-white tracking-tight">{cat.label}</h2>
                    <p className="text-xs text-gray-600 mt-0.5">{cat.desc}</p>
                  </div>
                </div>
                <span className="text-xs text-gray-700 bg-white/5 border border-white/8 rounded-full px-3 py-1 font-medium shrink-0 ml-4">
                  {cat.count} {cat.count === 1 ? 'tool' : 'tools'}
                </span>
              </div>
