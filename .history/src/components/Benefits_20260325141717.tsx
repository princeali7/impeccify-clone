    ...
    <section className="relative py-9 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="px-6 py-2 bg-[#151515] mx-auto block max-w-[260px] rounded-full">Benefits of Subscriptions</span>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map((benefit, idx) => (
            <div
              key={idx}
              className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 hover:border-transparent hover:bg-[#e3eb01] transition-all duration-200 group"
            >
              <div className="w-12 h-12 rounded-xl bg-white/[0.06] group-hover:bg-black/10 flex items-center justify-center mb-5 group-hover:scale-110 transition-all duration-200">
                <img src={benefit.icon} alt={benefit.title} className="w-7 h-7 group-hover:brightness-0 transition-all duration-200" />
              </div>
              <h3 className="text-lg font-bold mb-2 group-hover:text-black transition-colors duration-200">{benefit.title}</h3>
              <p className="text-gray-400 text-sm leading-relaxed group-hover:text-black transition-colors duration-200">{benefit.description}</p>
            </div>
          ))}
        </div>
      </div>
