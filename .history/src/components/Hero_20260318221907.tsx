...
export default function Hero() {
  return (
    <section className="relative pt-28 pb-20 overflow-hidden">
      ...
      <div
        className="relative mt-4 w-full overflow-hidden flex flex-col gap-3"
        style={{
      ...
      >
        ...
        <div className="flex gap-3">
          <div className="flex gap-3 animate-marquee shrink-0">
            {portfolioImages.map((img, i) => (
              <div key={i} className="rounded-xl overflow-hidden border border-white/[0.06] shrink-0" style={{ width: '280px', height: '180px' }}>
                <Image src={img.src} alt={img.alt} width={280} height={180} className="w-full h-full object-cover" unoptimized />
              </div>
            ))}
          </div>
          {/* duplicate for seamless loop */}
          <div className="flex gap-3 animate-marquee shrink-0" aria-hidden>
            {portfolioImages.map((img, i) => (
              <div key={i} className="rounded-xl overflow-hidden border border-white/[0.06] shrink-0" style={{ width: '280px', height: '180px' }}>
                <Image src={img.src} alt={img.alt} width={280} height={180} className="w-full h-full object-cover" unoptimized />
              </div>
        ...
        {/* Row 2 — scrolls right */}
        <div className="flex gap-3">
          <div className="flex gap-3 animate-marquee-reverse shrink-0">
          ...
          {/* duplicate for seamless loop */}
          <div className="flex gap-3 animate-marquee-reverse shrink-0" aria-hidden>
            {[...portfolioImages].reverse().map((img, i) => (
              <div key={i} className="rounded-xl overflow-hidden border border-white/[0.06] shrink-0" style={{ width: '280px', height: '180px' }}>
                <Image src={img.src} alt={img.alt} width={280} height={180} className="w-full h-full object-cover" unoptimized />
              </div>
